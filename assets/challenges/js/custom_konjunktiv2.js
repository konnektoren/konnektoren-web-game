        ...
        [
          ...
          [
            new Map([
              [
                "scenario",
                "Wenn ich ______ (sein) reich, ______ (reisen) um die Welt.",
              ],
              ["verb1", ["wäre", "bin", "war"]],
              ...
              [
                "hint",
                'Think about what you would "be" or "do" if you were rich.',
              ],
            ]),
            new Map([
              [
                "scenario",
                "Wenn er mehr Zeit ______ (haben), ______ (arbeiten) an seinem Projekt.",
              ],
              ...
              [
                ...
                new Map([
                  ["verb1", "hätte"],
                  ["verb2", "würde arbeiten"],
                ]),
              ],
              [
                "hint",
                'Think about what he would "have" or "do" if he had more time.',
              ],
            ]),
          ],
        ],
...
function checkAnswer() {
  ...
  verb1Select.querySelectorAll("option").forEach((option) => {
    option.style.backgroundColor =
      option.value === correctVerb1 ? "#388e3c" : "";
  ...
  verb2Select.querySelectorAll("option").forEach((option) => {
    option.style.backgroundColor =
      option.value === correctVerb2 ? "#388e3c" : "";
...
function finishChallenge() {
  ...
  if (window.konnektoren && window.konnektoren.executeCommand) {
    ...
    alert(
      `Challenge Finished! Your performance: ${((score / (totalQuestions * 10)) * 100).toFixed(2)}%`,
    );
  }
}
...
function collectUserAnswers() {
  ...
  questions.forEach((question, index) => {
    ...
    const verb2Select = document.getElementById("verb2");
    const selectedVerb1 =
      verb1Select.options[index + 1]?.value || "Keine Auswahl"; // Adjusting for 'Select' option
    const selectedVerb2 =
      verb2Select.options[index + 1]?.value || "Keine Auswahl"; // Adjusting for 'Select' option
    const correctVerb1 = question.get("correctAnswers").get("verb1");
    const correctVerb2 = question.get("correctAnswers").get("verb2");
