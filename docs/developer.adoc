...
The build environment configuration allows you to set various environment variables that are used throughout the application. These variables are set at compile time using a `build.rs` script.
...
* `KONNEKTOREN_CHAT_API_URL`: The URL for the chat API.
* `KONNEKTOREN_REVIEWS_API_URL`: The URL for the reviews API.
* `KONNEKTOREN_TON_API_URL`: The URL for the TON API.
* `KONNEKTOREN_TONCONNECT_MANIFEST_URL`: The URL for the TON Connect manifest.
* `KONNEKTOREN_PAYMENT_ADDRESS`: The payment address for TON transactions.

=== Setting Environment Variables

You can set these environment variables in your shell before building the project. For example:

[source,bash]
----
export BASE_PATH="/app"
export KONNEKTOREN_CHAT_API_URL="https://custom.api.url/api/v1/chat"
export KONNEKTOREN_REVIEWS_API_URL="https://custom.api.url/api/v1/reviews"
export KONNEKTOREN_TON_API_URL="https://custom.api.url/api/v1/ton"
export KONNEKTOREN_TONCONNECT_MANIFEST_URL="https://custom.url/assets/tonconnect-manifest.json"
export KONNEKTOREN_PAYMENT_ADDRESS="0:custompaymentaddress"
cargo build
----

If an environment variable is not set, a default value will be used as specified in the `build.rs` script.

=== `build.rs` Script

The `build.rs` script is responsible for setting the environment variables at compile time. Here is the content of the `build.rs` script:
...
The environment variables are accessed in the code using the `env!` macro. Here is an example from the `config.rs` file:
...
This setup ensures that the configuration values are set at compile time and can be easily changed by setting environment variables before building the project.

=== Example Usage

Here is an example of how the environment variables are used in the application:

[source,rust]
----
use crate::config::{CHAT_API_URL, REVIEWS_API_URL};

async fn fetch_reviews() -> Result<(), Box<dyn std::error::Error>> {
    let url = format!("{}/reviews", REVIEWS_API_URL);
    let resp = reqwest::get(&url).await?;
    let reviews: Vec<Review> = resp.json().await?;
    Ok(())
}

async fn send_chat_message(message: &str) -> Result<(), Box<dyn std::error::Error>> {
    let url = format!("{}/send", CHAT_API_URL);
    let client = reqwest::Client::new();
    let resp = client.post(&url).body(message.to_string()).send().await?;
    Ok(())
}
----

This structure allows for easy configuration and flexibility in managing environment variables for different environments (development, staging, production).
